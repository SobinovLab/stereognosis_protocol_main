...
class ArduinoE18Detector {
 public:
  ...
  explicit ArduinoE18Detector(std::string port = "", int baud = 115200)
      : port_(std::move(port)), baud_(baud) {}

  ~ArduinoE18Detector() { close(); }
  ...
  // Poll until detected stays true for >= stableMs.
 ...
 private:
  ...
  int ensureConnected() const {
    if (h_ == INVALID_HANDLE_VALUE) {
      // throw std::runtime_error("Serial not connected.");
      logError("Proximity sensor not connected.");
      return -1;
    }
    return 0;
  }
